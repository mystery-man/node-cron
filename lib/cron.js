...
function CronJob(cronTime, onTick, onComplete, startNow, timeZone, context, uniqueId) {
...
CronJob.remove = function(id){
	cronJobs.some(function(job, index){
		if(job.id === id){
			job.stopAndRemove(index);
			return true;
		}
	})
}
...
CronJob.prototype.stopAndRemove = function(index) {
	if (this._timeout)
		clearTimeout(this._timeout);
	this.running = false;
